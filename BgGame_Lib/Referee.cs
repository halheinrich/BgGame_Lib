...
///   2. Cube-response application: <see cref="CubeAction.Take"/> doubles the
///      cube and transfers ownership; <see cref="CubeAction.Pass"/> ends the
///      game with the offerer winning the pre-double cube value.
///
/// Play application is owned by <see cref="GameState.ApplyPlay(BgDataTypes_Lib.Play, int, int)"/>
/// rather than the Referee — turn-boundary mutation is a property of the
/// game state itself, not of an external arbiter, and the validated apply
/// primitive lives in BgMoveGen.
///
...
public sealed class Referee
{
    ...
    ///
    /// Called after <see cref="GameState.ApplyPlay(BgDataTypes_Lib.Play, int, int)"/>
    /// in normal flow, by which time perspective has flipped and the just-moved
    /// (winning) player is the "opponent" from the current perspective. The
    /// natural-flow case is therefore "opponent has 0 checkers; on-roll lost."
    /// The reverse case (on-roll has 0) is also handled for fixture /
    /// construction-from-finished-position scenarios.
    /// </summary>
